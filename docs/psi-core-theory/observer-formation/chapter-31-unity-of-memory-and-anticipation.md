---
sidebar_position: 31
title: "Chapter 31: The Unity of Memory and Anticipation"
---
...
Memory looks backward, anticipation looks forward. Yet in consciousness, they form an inseparable unity. Both are present-moment activities of $\psi$ relating to itself across time.
...
$$
\text{Memory}(t_{\text{past}}) = \psi_{\text{now}}(\psi_{t_{\text{past}}})
$$
...
$$
\text{Anticipation}(t_{\text{future}}) = \psi_{\text{now}}(\psi_{t_{\text{future}}}^{\text{potential}})
$$
...
$$
\text{Memory} \leftarrow \psi_{\text{now}} \rightarrow \text{Anticipation}
$$
...
$$
\text{Anticipation}_{t+1} = f(\text{Memory}_t, \text{Context}_t)
$$
...
$$
\text{Coherent self} = \int_{-\infty}^{+\infty} \psi(t) \cdot W(t) \, dt
$$

Where $W(t)$ weights past (memory) and future (anticipation). This integration creates the sense of being a temporally extended self.
...
$$
\text{Now} = \text{Memory} \cap \text{Anticipation}
$$
...
$$
\text{Déjà vu} = \text{Anticipation}_{t-\epsilon} \approx \text{Present}_t
$$

**Premonition**: Memory matches future
$$
\text{Premonition} = \text{Memory}_{t+\epsilon} \approx \text{Future}_t
$$
...
$$
\text{Self} = \text{Story}(\text{Memory} \to \text{Present} \to \text{Anticipation})
$$
...
$$
\text{Healing} = \text{Memory}_{\text{integrated}} + \text{Anticipation}_{\text{open}}
$$
...
$$
\text{Eternal now} = \lim_{\substack{\text{Memory} \to 0 \\ \text{Anticipation} \to 0}} \text{Present}
$$
