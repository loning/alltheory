---
sidebar_position: 13
title: "Chapter 13: The Necessity of Metalanguage"
---
...
To speak about language, we need language. This creates an immediate recursion—metalanguage emerges not as a luxury but as a necessity from the self-referential nature of $\psi = \psi(\psi)$.
...
L_1 &= \text{Metalanguage about } L_0 \\
L_2 &= \text{Meta-metalanguage about } L_1 \\
&\vdots \\
L_\infty &= \psi
\end{align}
$$

Each level can speak about the levels below it, and all levels collapse into $\psi$.
...
- Use: $\psi$ (direct application)
- Mention: "$\psi$" (reference to the symbol)

Yet both are operations within $\psi$:

$$
\text{Use}(\psi) = \psi(\cdot) \quad \text{and} \quad \text{Mention}(\psi) = \psi(\psi)
$$
...
$$
\text{Understand}(S) = \text{Meta}(\text{Use}(S))
$$
...
$$
\text{Describe}(\text{Language}) = \text{Language}(\text{Language}) = \psi(\psi)
$$
