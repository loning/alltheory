---
sidebar_position: 26
title: "Chapter 26: Observation as Participation"
---
...
Observation is not passive reception but active participation. In the $\psi$-framework, to observe is to engage in the creative act of reality formation.
...
$$
\text{Universe} = \sum_{\text{all observers}} O_i(\psi)
$$
...
$$
|\psi\rangle = \sum_i c_i|i\rangle \xrightarrow{O} |i_{\text{selected}}\rangle
$$
...
$$
\text{Reality}_O = \psi|_O = \text{The universe as seen by } O
$$

Yet all these realities are aspects of the same $\psi$:

$$
\psi = \bigcup_{\text{all } O} \psi|_O
$$
...
$$
\text{Creation} = \text{Observation} = \psi \to \psi'
$$
...
$$
O_{\text{now}}(\psi_{\text{past}}) = \psi'_{\text{past}}
$$
...
$$
\Delta \psi = O(\psi) - \psi = \text{The creative contribution of } O
$$
...
$$
\text{Reality}_{\text{consensus}} = \bigcap_{i} O_i(\psi)
$$
...
$$
O(\psi) \in \{\psi' : \psi' \text{ is consistent with } \psi = \psi(\psi)\}
$$
...
$$
\text{Participation} \supset \text{Conscious choice}
$$
...
When $\psi$ observes itself:

$$
\psi(\psi) = \psi' \text{ where } \psi' \text{ includes self-observation}
$$
