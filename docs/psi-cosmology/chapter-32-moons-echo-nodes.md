---
title: "Chapter 32: ψ-Moons as Collapse Echo Nodes"
sidebar_label: "32. ψ-Moons as Collapse Echo Nodes"
---
...
**Definition 32.1** (Collapse Node):
Echo nodes occur where:
...
**Theorem 32.1** (Nodal Accretion):
Mass accumulation rate at nodes:
$$\frac{dM}{dt} = 4\pi r^2 \rho v_{rel} P_{capture}$$

where P_capture = exp(-E_rel/ψ_node).

*Proof*:
Particles crossing echo nodes experience enhanced collapse binding. Those with kinetic energy below the node potential remain trapped, gradually accumulating into moons. The capture probability depends exponentially on the ratio of kinetic to potential energy. ∎

## 32.3 Resonant Chain Architecture

Moons form harmonic sequences:

**Definition 32.2** (Laplace Resonance):
Multiple moons satisfy:
...
**Theorem 32.2** (Synchronous Rotation):
Spin-orbit locking occurs when:
...
**Definition 32.3** (Capture Criterion):
External objects capture when:
...
**Theorem 32.3** (Mass Distribution):
Moon masses follow:
...
**Definition 32.4** (Roche Processing):
...
**Definition 32.5** (Melt Criterion):
Oceans exist when:
...
**Theorem 32.5** (Hierarchical Stability):
Submoons survive when:
