...
**证明**：设T: ψ-Cat₁ → ψ-Cat₂保持ψ。必须证明：
1. T(id_A) = id_{T(A)}：由于T保持ψ结构，
   $$T(\text{id}_A)(\psi) = T(\psi) = \text{id}_{T(A)}(T(\psi))$$
2. T(g∘f) = T(g)∘T(f)：由ψ保持性，
...
对于任何目标T和映射f: S → T，通过ψ*的因子分解是唯一的，因为ψ*恰好捕获公共ψ结构。唯一性从最优性得出。∎
...
真值态射t: 1 → Ω挑出"真"。对于任何单态m: A ↪ B，唯一χ_m使拉回方块。这给出ψ模式的内部逻辑。∎
